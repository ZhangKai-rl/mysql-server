...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│   Flush 线程正在遍历 flush_list：                                            │
│   ┌────┐    ┌────┐    ┌────┐    ┌────┐                                      │
│   │ A  │───→│ B  │───→│ C  │───→│ D  │                                      │
│   └────┘    └────┘    └────┘    └────┘                                      │
│               ↑                                                              │
│               │ flush 线程记住了 prev = B，准备刷 C                          │
│                                                                              │
│   此时，另一个线程把 B 从 flush_list 中移除：                                │
│                                                                              │
│   ┌────┐              ┌────┐    ┌────┐                                      │
│   │ A  │─────────────→│ C  │───→│ D  │                                      │
│   └────┘              └────┘    └────┘                                      │
│               ↑                                                              │
│               │ flush 线程的 prev 指针现在指向已脱离链表的 B！               │
│                                                                              │
│   危险！继续使用 prev 会导致遍历中断或访问无效内存                           │
└─────────────────────────────────────────────────────────────────────────────┘
...
│                                                                                 │
│ ═══════════════════════════════════════════════════════════════════════════════│
│   Flush 线程 (Thread 1)                     Other 线程 (Thread 2)              │
│ ───────────────────────────────────────     ──────────────────────────────────│
│                                                                                 │
│   Step 1: 获取 flush_list_mutex ✓                                              │
│                                                                                 │
│   Step 2: 遍历到 C，准备处理                                                    │
│      bpage = C                                                                  │
│      prev = UT_LIST_GET_PREV(list, C) = B                                       │
│      buf_pool->flush_hp.set(B)                                                  │
│                                                                                 │
│      此时内存状态:                                                               │
│      ┌─────────────────────────────────┐                                       │
│      │  buf_pool->flush_hp.m_hp = B    │  ← HP 指向 B                          │
│      └─────────────────────────────────┘                                       │
│                                                                                 │
│   Step 3: 释放 mutex，对 C 进行 I/O                                             │
│      buf_flush_list_mutex_exit()                                               │
│      // ... 进行异步 I/O ...                                                    │
│                                                                                 │
│                                           Step 4: 获取 flush_list_mutex ✓      │
│                                           要把 B 从 flush_list 移除            │
│                                                                                 │
│                                           Step 5: buf_flush_remove(B) 内部:    │
│                                                                                 │
│                                           // ★★★ 先调整 HP ★★★               │
│                                           buf_pool->flush_hp.adjust(B);        │
│                                                                                 │
│                                           adjust(B) 执行过程:                   │
│                                           ┌───────────────────────────────┐    │
│                                           │ if (m_hp == B) {  // TRUE     │    │
...
│                                           此时内存状态变为:                      │
│                                           ┌─────────────────────────────────┐  │
│                                           │  buf_pool->flush_hp.m_hp = A    │  │
│                                           └─────────────────────────────────┘  │
│                                                                                 │
│                                           Step 6: 真正从链表移除 B              │
│                                           UT_LIST_REMOVE(flush_list, B);       │
│                                                                                 │
│                                           链表变为:                              │
│                                           ┌────┐    ┌────┐    ┌────┐          │
│                                           │ A  │←───│ C  │←───│ D  │          │
...
│                                                                                 │
└────────────────────────────────────────────────────────────────────────────────┘
与server LF_HASH LF_PIN的区别：
特性	InnoDB Buffer Pool	LF_HASH
目标	链表遍历时保护迭代器	无锁哈希表的内存安全回收
保护方式	协作式（修改者主动调整 hp）	声明式（扫描所有 pins 后才能释放）
xxxx: 注意这里，修改者可以删除block而不是lf_hash的延迟删除，然后将prev置为prev->prev(buf_pool->flush_hp.m_hp)避免aba问题
并发策略	使用 mutex 保护	完全 Lock-Free
内存回收	不负责内存回收	延迟释放（Purgatory）
Pins 数量	每个 buf_pool 固定几个	每线程固定 4 个
实现复杂度	简单	复杂（需要扫描所有线程）
 */
class HazardPointer {
 public:
  /** Constructor
