...
// 每个索引在bp中的页面数量
/** sql变长key, innodb定长64b key.
┌───────────────────────────────────────────────────────────────────────────┐
│                   ut_lock_free_hash_t (InnoDB)                            │
├───────────────────────────────────────────────────────────────────────────┤
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐                   │
│  │ arr_node_1  │───>│ arr_node_2  │───>│ arr_node_3  │──> NULL           │
│  │ (初始数组)   │    │ (扩容后新建) │    │ (再次扩容)   │                   │
│  ├─────────────┤    ├─────────────┤    ├─────────────┤                   │
│  │[(k1,v1)]    │    │[(k1,GOTO)]  │    │[(k1,v1)]    │                   │
│  │[(k2,v2)]    │    │[(k2,GOTO)]  │    │[(k2,v2)]    │                   │
...
│  └─────────────┘    └─────────────┘    └─────────────┘                   │
│                                                                           │
│  特点：链表式数组扩容，旧数组迁移到新数组                                    │
└───────────────────────────────────────────────────────────────────────────┘

┌───────────────────────────────────────────────────────────────────────────┐
│                        LF_HASH (SQL层)                                    │
├───────────────────────────────────────────────────────────────────────────┤
│  LF_DYNARRAY (4级动态数组，每级256元素，最大 4,311,810,304 桶)              │
│                                                                           │
│  全局 Split-Ordered List (按 bit-reversed hash 排序，仅一条):               │
│  ┌───────────────────────────────────────────────────────────────────────┐ │
│  │  d0 ─−→ d1 ─−→ node(bkt1) ─−→ d2 ─−→ node(bkt2) ─−→ ...              │ │
│  │   ↑       ↑                  ↑                                        │ │
│  │  bkt[0]  bkt[1]            bkt[2] (lazy init via initialize_bucket)   │ │
│  └───────────────────────────────────────────────────────────────────────┘ │
│                                                                           │
│  桶指针只是全局链表的"入口"，指向 dummy 节点                                 │
│  dummy 节点(hashnr bit0=0): 哨兵，永不删除，作安全锚点                       │
│  node  (hashnr bit0=1): 普通节点，link bit0=DELETED 标记                   │
│                                                                           │
│  特点：Split-Ordered List + Hazard Pointer (Shalev&Shavit 2003 / Michael 2004) │
└───────────────────────────────────────────────────────────────────────────┘
 */
