...
- [核心实现](#核心实现)
  ...
  - 主库侧：依赖追踪
  - 从库侧：MTS 调度
  - 提交保序：replica_preserve_commit_order（独立文档 [preserve_commit_order.md](preserve_commit_order.md)）
  - 完整时序图
  - WL#7165 经典图解分析
  - Writeset Hash 生成机制
  - GAQ 与 Checkpoint 机制
...
#### 假阴性的兜底：replica_preserve_commit_order

RBR 消解了存储层的执行因果，但两个口子仍然开着：① 客户端读从库仍可能看到违反因果的中间态（T2 先提交、T1 后提交的窗口）；② 乱序提交会写进从库自己的 binlog，级联复制下游继承错误顺序。MySQL 8.0 起用 `replica_preserve_commit_order`（默认 ON）兜底：worker 可以乱序执行，但提交必须按 relay log 顺序——用"取号排队"替代因果分析，先派发先提交。

完整剖析（为什么用位置替代因果 / 三段接力 / Commit_order_manager 的 FIFO 队列与 MDL 复用 / 两条提交路径 / 缺陷档案 #87796、#103636 / 性能与观测）见 [`preserve_commit_order.md`](preserve_commit_order.md)。
...
> Layer 4 的完整实现（Commit_order_manager / 缺陷档案 / 性能与观测）见 [`preserve_commit_order.md`](preserve_commit_order.md)。
...
#### 运行期清理：rotate() 与 tracking_mode_changed()

`Writeset_trx_dependency_tracker::rotate(int64 start)` 本体只有两行，但**调用它的两个时机语义完全不同**：

```cpp
void Writeset_trx_dependency_tracker::rotate(int64 start) {
  m_writeset_history_start = start;   // 把"历史最老水位"抬到 start
  m_writeset_history.clear();         // 全清
}
```

**① binlog rotate 时**（`Transaction_dependency_tracker::rotate()`）：

```cpp
m_commit_order.rotate();      // 更新 offset（见「核心概念」）
/*
...
*/
m_writeset.rotate(1);         // ★ 水位抬到 1，而不是当前 sn
```

`m_commit_order.rotate()` 之后 `m_writeset.rotate(1)` 把水位设为 **1**——注释说得很直白：**让从库在轮转后能并行执行**。轮转是天然同步栅栏（见 Q4），跨文件必然无依赖，所以直接把水位压到最小值 1，新文件里的事务只要没命中历史行就能拿到 `lc=1`（最小依赖）。若这里传的是"当前 sn"，新文件的事务就会被人为挂在一个并不存在的依赖后面，白白损失并行度。

**② 运行期切换追踪模式时**（`tracking_mode_changed()`）：
...
int64 timestamp = max_committed_transaction.get_timestamp() -
                  max_committed_transaction.get_offset();   // 相对值
m_writeset.rotate(timestamp);                              // 水位 = 当前已提交水位
```

这里传的是**当前已提交水位的相对值**——与 rotate 场景相反，是**保守**的。原因：切换前跑的是别的模式（如 COMMIT_ORDER），**history 从来没维护过**；若切换后直接沿用空 history，新事务会误判"我没碰到任何历史行 → 与之前所有事务无冲突"（假阴性！）。把水位抬到"当前已提交水位"，就保证了切换后第一个事务至少依赖所有已提交事务——**宁可牺牲并行度也不冒假阴性的风险**。

对比记忆：同一个 `rotate()` 函数，轮转时传 1（激进，因为栅栏保证安全），改模式时传当前水位（保守，因为历史不可信）。
```
